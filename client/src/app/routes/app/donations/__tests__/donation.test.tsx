import { default as DonationRoute } from '../donation';
...
const renderDonation = async () => {
  ...
  const fakeDonation = await createDonation({ teamId: fakeUser.teamId });

  const utils = await renderApp(<DonationRoute />, {
    user: fakeUser,
    path: `/app/donations/:donationId`,
    url: `/app/donations/${fakeDonation.id}`,
  });

  await screen.findByText(fakeDonation.title);
...
test('should render donation', async () => {
  const { fakeDonation } = await renderDonation();
  expect(screen.getByText(fakeDonation.body)).toBeInTheDocument();
});

test('should update donation', async () => {
  const { fakeDonation } = await renderDonation();
  ...
  await userEvent.click(
    screen.getByRole('button', { name: /update donation/i }),
  );

  const drawer = await screen.findByRole('dialog', {
    name: /update donation/i,
  });

  const titleField = within(drawer).getByText(/title/i);
  const bodyField = within(drawer).getByText(/body/i);

  const newTitle = `${fakeDonation.title}${titleUpdate}`;
  const newBody = `${fakeDonation.body}${bodyUpdate}`;
...
test(
  'should create and delete a comment on the donation',
  async () => {
